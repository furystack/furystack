...
- Rewrote JSDoc on `Cache`, `CacheSettings`, and `CacheResult` to follow the new value-test guidance: dropped restate-the-type narration, tightened wording on `getKey` / `setExplicitValue` / `getObservable` semantics, and added an `@example` for `Cache` showing the `using(...)` + `subscribe(...)` flow.
...
- Version bumped to align with the monorepo-wide functional DI migration. No DI surface in this package; the `Cache` primitive and its exports are unchanged. See the [v7 migration guide](../../docs/migrations/v7-functional-di.md) for the full picture on other packages.

## [6.1.5] - 2026-04-17

### ⬆️ Dependencies

- Raised `typescript` to ^6.0.3 and `vitest` to ^4.1.4 so package builds and tests track the workspace compiler and test runner.
