...
export class Repository implements Disposable, IApi {
  ...
  public async Find<T>(options: {
          ...
          for (const key of Object.keys(options.data)) {
            qb = qb.where(
              new Brackets((fieldBracket) =>
                fieldBracket
                  .where("ContentField.Name = :name", { name: key })
                  .andWhere("ContentField.Value = :value", {
    ...
    if (options.contentType) {
      query = query
        ...
        .andWhere("contentType.name = :contentTypeName", {
    ...
    const contentIds = result
      .filter((c) => c.Content && c.Content.Id)
      .map((c) => c.Content.Id);
    return await this.Load({ ids: contentIds, aspectName: options.aspectName });
  }

  public async Create<T>(options: {
    contentType: Constructable<T>;
    data: T;
  }): Promise<ISavedContent<T>> {
    const contentTypeName = options.contentType.name;
    const currentUser = await this.userContext.GetCurrentUser();
    return await this.GetManager().transaction(async (tr) => {
      const contentType = await tr.findOneOrFail(
        this.options.models.ContentType,
      ...
      if (
        ...
        if (
          ...
          throw Error(
            `No 'Create' permission for content type '${contentTypeName}'`,
          );
        }
      }
      ...
      const fields = Object.keys(options.data).map((field) => {
        const fieldDef =
          contentType.Fields &&
          contentType.Fields[field as keyof typeof contentType["Fields"]];
        if (fieldDef && fieldDef.Type !== "Value") {
          if (fieldDef.Type === "Reference") {
            const fieldValue = (options.data as any)[
              field
            ] as ISavedContent<{}>;
            (options.data as any)[field] = JSON.stringify(fieldValue.Id);
          }
          if (fieldDef.Type === "ReferenceList") {
            const fieldValue = (options.data as any)[field] as Array<
              ISavedContent<{}>
            >;
            (options.data as any)[field] = JSON.stringify(
              fieldValue.map((f) => f.Id),
            );
          }
        }
    ...
    return await Promise.all(
      filtered.map(async (c) =>
        this.aspectManager.TransformPlainContent<T>({
          ...
          loadRef: async (ids) =>
            await this.Load({ ids, aspectName: DefaultAspects.Expanded }),
        }),
      ),
    );
  }
  ...
  private async initConnection() {
    ...
    try {
      ...
      if (
        ...
        !cm.has(this.options.connection.name)
      ) {
  ...
  public async Update<T>(options: {
    ...
    return await this.GetManager().transaction(async (tm) => {
      ...
      const currentUser = await this.userContext.GetCurrentUser();
      const isAdmin = await this.roleManager.HasRole(
        currentUser,
        this.systemContent.AdminRole,
      );
      ...
      const aspect = this.aspectManager.GetAspectOrFail(
        existingContent,
        aspectName,
      );
      const validationResult = this.aspectManager.Validate(
        existingContent,
        options.change,
        aspect,
      );
      if (!validationResult.isValid) {
        throw Error(
          `Content update is not valid. Details: ${JSON.stringify(
            validationResult,
          )}`,
        );
      }
      for (const field of existingContent.Fields) {
        ...
        if (changeValue !== field.Value) {
          field.Value =
            (changeValue && changeValue.toString && changeValue.toString()) ||
            (null as any);
          await tm.save(field);
        }
      }
  ...
  public GetPhysicalStoreForType = <TM extends ISavedContent<any>>(
    contentType: Constructable<TM>,
  ) =>
    ({
      primaryKey: "Id",
      logger: this.logger,
      ...
      filter: async (data, aspectName = DefaultAspects.Details) =>
        await this.Find({ data, contentType, aspectName }),
