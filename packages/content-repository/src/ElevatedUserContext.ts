...
@Injectable()
export class ElevatedUserContext<
  TUser extends ISavedContent<IUser> = ISavedContent<User>
> implements UserContext<TUser>, Disposable {
  ...
  public async GetCurrentUser(): Promise<TUser> {
    ...
    return (await this.injector.options.parent
      .GetInstance(UserContext)
      .GetCurrentUser()) as TUser;
  }
  constructor(
  ...
  public static Create<
    TUser extends ISavedContent<IUser> = ISavedContent<User>
  >(injector: Injector): ElevatedUserContext<TUser> {
    injector.Remove(ElevatedUserContext);
    const instance = injector.GetInstance<ElevatedUserContext<TUser>>(
      ElevatedUserContext,
      true,
    );
