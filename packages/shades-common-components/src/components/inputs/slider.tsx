...
export const Slider = Shade<SliderProps>({
  ...
  render: ({ props, injector, element, useDisposable }) => {
    useDisposable('pointer-drag-handler', () => {
      ...
      const getProps = (): SliderProps & { min: number; max: number; step: number } => {
        ...
        return { ...p, min: p?.min ?? 0, max: p?.max ?? 100, step: p?.step ?? 1 }
      }
      ...
      const handlePointerDown = (e: MouseEvent | TouchEvent): void => {
        ...
        if (isThumb) {
          ...
          const currentValue = p.value ?? p.min
